...
export default function WordIcon({ word, size = 72, photoSize = 180, inline = false }) {
  const photoUrl = getWordPhotoUrl(word)
  const marginOverride = inline ? { margin: 0, flexShrink: 0 } : null
  if (photoUrl) {
    return (
      <div className="word-icon-photo" style={{ width: photoSize, height: photoSize, ...marginOverride }}>
  ...
  return (
    <div className="word-icon" style={{ width: size, height: size, ...marginOverride }}>
